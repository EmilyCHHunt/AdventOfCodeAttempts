...
def generateInfiniteMap(oldMap):
    totalInfinites = 0;
    for y in range(len(oldMap)):
        for x in range(len(oldMap[y])):
            copyOfMap = copy.deepcopy(oldMap);
            if copyOfMap[y][x] != "^" and copyOfMap[y][x] != "#":
                copyOfMap[y][x] = "#";
            if simulate(copyOfMap):
                totalInfinites += 1;
    print(totalInfinites);
